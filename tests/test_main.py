...
def run_command(options: str) -> Tuple[str, dict, dict]:
...
def test_legacy():
    ...
    url, headers, data = run_command(
        "--cf-zone  --cf-auth  --urls  --tags  --hosts  --prefixes "
    )
...
def test_new():
    ...
    url, headers, data = run_command(
        "--cf-zone zone123 --cf-auth key123 --urls  --tags  --hosts  --prefixes "
    )
...
def test_new_missing():
    ...
    with pytest.raises(Exception):
        run_command("--cf-zone  --cf-auth  --urls  --tags  --hosts  --prefixes ")

    with pytest.raises(Exception):
        run_command("--cf-zone zone123 --cf-auth  --urls  --tags  --hosts  --prefixes ")

    with pytest.raises(Exception):
        run_command("--cf-zone --cf-auth key123 --urls  --tags  --hosts  --prefixes ")
...
def test_mix():
    ...
    url, headers, data = run_command(
        "--cf-zone zone123 --cf-auth  --urls  --tags  --hosts  --prefixes "
    )
    assert url == "https://api.cloudflare.com/client/v4/zones/zone123/purge_cache"
    assert headers["Authorization"] == "Bearer key123"
    assert data == {"purge_everything": True}
    ...
    url, headers, data = run_command(
        "--cf-zone  --cf-auth key123 --urls  --tags  --hosts  --prefixes "
    )
    assert url == "https://api.cloudflare.com/client/v4/zones/zone123/purge_cache"
    assert headers["Authorization"] == "Bearer key123"
    assert data == {"purge_everything": True}
...
def test_urls():
    url, headers, data = run_command(
        "--cf-zone zone123 --cf-auth key123 --urls nathanv.me google.com --tags  --hosts  --prefixes "
    )
...
def test_tags():
    url, headers, data = run_command(
        "--cf-zone zone123 --cf-auth key123 --urls  --tags tag1 tag-2 --hosts  --prefixes "
    )
...
def test_hosts():
    url, headers, data = run_command(
        "--cf-zone zone123 --cf-auth key123 --urls  --tags  --hosts nathanv.me google.com --prefixes "
    )
...
def test_prefixes():
    url, headers, data = run_command(
        "--cf-zone zone123 --cf-auth key123 --urls  --tags  --hosts  --prefixes nathanv.me/assets https://google.com/images"
    )
...
def test_full():
    ...
    url, headers, data = run_command(
        "--cf-zone zone123 --cf-auth key123 --urls nathanv.me google.com --tags tag1 tag-2 --hosts nathanv.me google.com --prefixes nathanv.me/assets https://google.com/images"
    )
...
def test_cli():
    ...
    url, headers, data = run_command(
        "--cf-zone zone123 --cf-auth key123"
    )
    assert url == "https://api.cloudflare.com/client/v4/zones/zone123/purge_cache"
    assert headers["Authorization"] == "Bearer key123"
    assert data == {"purge_everything": True}
